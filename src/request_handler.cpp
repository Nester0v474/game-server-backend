...
namespace http_handler {
    ...
    namespace {
        ...
        json::object SerializePlayer(const model::Player& player, const model::Dog& dog) {
            json::object player_json;

            player_json["pos"] = json::array({ dog.GetPosition().x, dog.GetPosition().y });

            
            auto velocity = dog.GetVelocity();
            player_json["speed"] = json::array({ velocity.vx, velocity.vy });
            ...
            for (const auto& item : dog.GetBag()) {
                ...
                bag_json.push_back(std::move(item_json));
            }
        ...
        json::object SerializeLostObject(const model::LootItem& loot) {
            json::object loot_json;
            loot_json["type"] = loot.GetType();
            loot_json["pos"] = json::array({ loot.GetPosition().x, loot.GetPosition().y });
            return loot_json;
        }
    ...
    void RequestHandler::HandleApiRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (target.starts_with("/api/v1/game/join")) {
            ...
            HandleGetRecords(std::forward<decltype(req)>(req), std::forward<Send>(send));
        }
    ...
    template <typename Body, typename Allocator, typename Send>
    void RequestHandler::HandleGetRecords(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        if (!database_) {
            SendErrorResponse(req, std::forward<Send>(send),
                http::status::internal_server_error,
                "internalServerError", "Database not available");
            return;
        }
        ...
        if (query_pos != std::string_view::npos) {
            ...
            if (start_pos != std::string_view::npos) {
                ...
                std::string_view start_str = query.substr(start_pos + 6);
                if (start_end != std::string_view::npos) {
                    start_str = start_str.substr(0, start_end - start_pos - 6);
                }
                try {
            ...
            size_t max_items_pos = query.find("maxItems=");
            if (max_items_pos != std::string_view::npos) {
                size_t max_items_end = query.find('&', max_items_pos);
                std::string_view max_items_str = query.substr(max_items_pos + 9);
                if (max_items_end != std::string_view::npos) {
                    max_items_str = max_items_str.substr(0, max_items_end - max_items_pos - 9);
                }
                try {
                    max_items = std::stoi(std::string(max_items_str));
                    if (max_items < 0) {
                        max_items = 100;
                    }
                } catch (...) {
        ...
        if (max_items > 100) {
            SendErrorResponse(req, std::forward<Send>(send),
                ...
                "badRequest", "maxItems cannot exceed 100");
            return;
        }
        ...
        for (const auto& record : records) {
            ...
            record_json["score"] = record.score;
            record_json["playTime"] = record.play_time_seconds;
            records_json.push_back(std::move(record_json));
        }
    ...
    template void RequestHandler::HandleGetRecords<http::string_body, std::allocator<char>,
        std::function<void(http::response<http::string_body>&&)>>(
            http::request<http::string_body, http::basic_fields<std::allocator<char>>>&&,
            std::function<void(http::response<http::string_body>&&)>&&);
